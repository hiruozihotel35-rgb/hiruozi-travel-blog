...
function profileImageExists() {
  ...
  const profileImagePath = path.join(process.cwd(), "public", siteConfig.profileImage.replace(/^\/+/, ""));
  return fs.existsSync(profileImagePath);
}

export function AuthorBox({ compact = false }: AuthorBoxProps) {
  const hasProfileImage = profileImageExists();
    ...
    <section className="grid gap-5 rounded-lg border border-stone-200 bg-white p-5 sm:grid-cols-[96px_1fr]">
      <div className="relative h-24 w-24 overflow-hidden rounded-full bg-stone-100">
        {hasProfileImage ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-stone-900 text-lg font-semibold text-white">
            {siteConfig.authorName.slice(0, 2)}
          </div>
        )}
      </div>
      <div>
